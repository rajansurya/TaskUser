...
public class DisplayTaskListAdapter extends ArrayAdapter<TaskInfoEntity> {
	private LayoutInflater mInflater;
	
	private AllTaskActivity mActivity;
	private ArrayList list_entity;
	private HashMap<Integer, View> viewMap;
	
	private TaskInfoEntity mTaskInfoEntitySelected;

	public DisplayTaskListAdapter(AllTaskActivity activity, ArrayList list, ListView listView) {
...
//		if(DBAdapter.getInstance(mActivity).isUserRegistered(assignFromNum)){
//			holder.nameTextView.setTextColor(mActivity.getResources().getColor(R.color.black));
//		} else {
//			holder.nameTextView.setTextColor(mActivity.getResources().getColor(R.color.grey));
//		}
		...
		holder.focusNowButton.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				...
				if("Favorite".equalsIgnoreCase(mActivity.headerTitle)){
					...
					DisplayTaskListAdapter.this.notifyDataSetChanged();
		...
		holder.nameTextView.setOnClickListener(new  OnClickListener() {
			...
			public void onClick(View v) {
				...
				try {
					...
					list_entity.clear();
					list_entity.addAll((ArrayList<TaskInfoEntity>) entities.get("list"));
					mActivity.setHeaderTitleCount(name, list_entity.size());
...
//			Toast.makeText(mActivity, word, Toast.LENGTH_SHORT).show();			
	...
	public void showMoreDialog(final ViewHolder holder, final TaskInfoEntity entity) {
...
//			SMSButton.setText("Open");
	...
	public void clickOnCloseButton(final ViewHolder holder, final TaskInfoEntity entity){
		STATUS = "";
		final String targetDate = entity.getTarget_date();
		final String AssignFromNum = entity.getUserInfoFrom().getMobile_number();
...
//	    sendIntent.setType("message/rfc822");
	...
	public void clickOnLaterButton(final ViewHolder holder, final TaskInfoEntity entity){
		try{
			ContentValues initialValues = new ContentValues();
			if("Y".equalsIgnoreCase(entity.getIsTaskSnooz())){
